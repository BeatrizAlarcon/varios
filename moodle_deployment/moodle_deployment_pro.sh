...
############################################################################
# Moodle-based web apps deployment script                                  #
#                                                                          #
#                                                                          #
...
if [ ${#ARGS[*]} -lt 3 ]; then
  ...
  echo "Usage: bash moodle_deployment_pro.sh [SVN_REPO] [USER] [SERVER]"
  exit -1;
else
	SVN_REPO=${ARGS[0]}
	USER=${ARGS[1]}
...
if [[ "$VERSION_NAME" != *rev$REVISION_NUMBER* ]]; then
	...
	REVISION_NUMBER=$REVISION_NUMBER_INPUT
...
CONFIG_FILES_DIR=$TMP_DIR_NAME"/config/config_files/pro"
...
$SSH_PREFIX svn co --username="$SVN_USERNAME" --password="$SVN_PASSWORD" "$SVN_REPO""/config/config_files/pro@$REVISION_NUMBER" $TMP_DIR_NAME"/config/config_files/pro" "> $TMP_SVN_LOG"

# Delete hidden files
echo "$LOG_MARK Deleting hidden files in release directory"
$SSH_PREFIX "rm -rf \`find $TMP_DIR_NAME/ -name '\.*'\` "
